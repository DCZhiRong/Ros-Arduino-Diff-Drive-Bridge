...
void setup() {
  ...
  for (int i = 0; i < sizeof(motor)/sizeof(Servo); i ++)
  {
    mysensor[i].begin(); 
...
void loop() {
  ...
  switch(usersetting)
  {
    case 'e':
      ...
      break;
    case 'p':
      motor[0].write(speedInput1);
      motor[1].write(speedInput2);
      break;
  }
  ...
  if (Serial.available()>0) { //keep asking for new user input without pausing
      ...
      switch(usersetting)
      {
        case 'e':
          ...
          rpsInput1 = -(rpsInput1)/(M_PI);
          rpsInput2 = (rpsInput2)/(M_PI);
          //e_integral1 = 0.0;
          //pre_error1 = 0.0;
          //e_integral2 = 0.0;
          //pre_error2 = 0.0;
        ...
        case 'a':
          usersetting = pre;
          Serial.print(-((pos1*M_PI)/360));
          Serial.print(" ");
          Serial.println(((pos2*M_PI)/360));
          break;
      }
    }
  }
...
void motor_control(float rpsMeasured, float rpsInput, float *speedInput, float *e_integral, float *pre_error, unsigned long *prevT, Servo motor){
  ...
  *speedInput = map(*speedInput, -255, 255, 0, 180);
  //Serial.println(*speedInput);
  motor.write(*speedInput);
}
